...
export default function BookEvent() {
  ...
  // 2) Live availability (server truth)
  const {
    data: availability,
  ...
  const seatsLeft =
    Number.isFinite(availability?.seatsLeft) ? availability.seatsLeft : null;
  ...
  const { mutate, isLoading: isBooking } = useMutation({
    ...
    onSuccess: async () => {
      ...
      // ensure lists refresh immediately
      await queryClient.invalidateQueries({ queryKey: ["my-bookings"] });
      await queryClient.invalidateQueries({ queryKey: ["availability", id] });
